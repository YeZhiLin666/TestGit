...
#define  LBT21024FR_FINE_TUNE_MAXADJUSTFACTOR    0.02f  // Modified by hardware Engieer 20140617 微调

#define  LBT21024FR_SETTING_TIME                 500       //电流上升沿时间，单位为100us
...
St_DisposableTask LBT21024FR_Base::SetCCToCC[4];    //加入了多range切换，所以这里扩充到10步
...
// const St_Port LBT21024FR_Base::Pin_RedundantSafety= {1,   6,        eCPTYPE_MCUGPIO,   GPIO_I,                0};//用于中电，MAXWLL 
//#if(FAN_SAFETY_MODE==1)
...
{  
	...
	CPLD_FUNC::WriteDOBus(0,LBT21024FR_relaystatus[0].Reg);  //硬件改变控制时序，改成高有效 20140331
...
// 	LBT21024FR_TempSample::AD_Init(1);//add by qjm20140613
	...
	AdditionalEngine();
    LBT21024FR_ADC5DAC::Dac_out_Clamp_Vlt(9,DAC_OUT_PUT_MIN);   // 正10V
		LBT21024FR_ADC5DAC::Dac_out_Clamp_Vlt(11,DAC_OUT_PUT_MIN);
	  LBT21024FR_ADC5DAC::Dac_out_Clamp_Vlt(8,DAC_OUT_PUT_MAX);   //负向最大
...
void LBT21024FR_Base::Software_Init(void)
{
	...
	Step_Manager_task::m_CurFineTuneFactor.m_fMinError  = LBT21024FR_FINE_TUNE_MINERRORFACTOR;    //  20140617 微调
...
// 	board_config::Func_TempSample = &LBT21024FR_TempSample::Temp_rd;  // 20140613  温度测量
	board_config::Func_RedundantSafety= &LBT21024FR_Base::RedundantSafety; //冗余电压保护
	board_config::Func_ReleaseRedundantSafety=&LBT21024FR_Base::ReleaseRedundantSafety;//冗余解除操作
    board_config::Func_FanAdj=&LBT21024FR_Base::FanAdjOperature;	//added by NiuYasong
	//#if(FAN_SAFETY_MODE==1)
	if(board_config::Feature_Enable_init->Fan_Safety_Mode==1)
	{
	   board_config::Func_FanSafety= &LBT21024FR_Base::FanCheckSafety; //风扇保护
  }  //#endif	
...
//             break;
        case ACTIVE_DAC_VH:
             ...
             LBT21024FR_ADC5DAC::Dac_out_Clamp_Vlt(chn,Data);   // 20140613  电压保护
             break;
        case ACTIVE_DAC_VL:
				    if(ChannelIdx==0)
							 chn=8;
