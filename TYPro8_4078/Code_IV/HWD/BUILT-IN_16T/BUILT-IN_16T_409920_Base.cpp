...
void BuiltIn_16T_409920_Base::Hardware_Init(unsigned char BoardID)
{
    ...
    BuiltIn_DataManager::BuiltIn_ADC_Info[0].b_ADCCH_Valid[16]=1; //±£Ö¤²ÉÑùÎÂ¶È
// 	  BuiltIn_DataManager::BuiltIn_ADC_Info[0].m_BoardADC_OpenADG = &BuiltIn_16T_409920_Base::BuiltIn_2ndV_ADG_Open;
    ...
    for(int i = 0; i< 16; i++) //¸Ã°åÐÍµ×°åÊÇ16Í¨µÀÎÂ¶È£¬ÉÏÃæ´ø¸±°å¿ÉÒÔÖ§³Ö¶àÖÖÀàÐÍ¡£
    {
    ...
    //                       BuiltIn_DataManager::BuiltIn_ADC_Info[BoardID].m_BoardType = Aux_BulitIn_16V_409386;
                    ...
                    BuiltIn_DataManager::BuiltIn_ADC_Info[1].m_ADCCH_MaxCnt = 17;
    					      BuiltIn_DataManager::BuiltIn_ADC_Info[1].b_ADCCH_Valid[16]=1; //±£Ö¤²ÉÑùÎÂ¶È
...
unsigned long BuiltIn_16T_409920_Base::BuiltIn_TC_Read(unsigned char u_ADCid,unsigned char ADCchn)
{
    ...
    if (ADCchn>=16)
    {
        ...
        if(ul_Inchip_temperature!=0)
        {
            if(u_ADCid ==0)
            {
                BuiltIn_DataManager::m_InChipTemperature[0].InChipTemp_instant_code = ul_Inchip_temperature ;//½«Êý¾Ý·Åµ½Ò»¸öÍ³Ò»¹ÜÀíµÄÆ¬ÄÚÎÂ¶ÈQUEÀïÒÔ±ã×öÆ½¾ù´¦Àí
                BuiltIn_DataManager::m_InChipTemperature[0].updated= true ;//½«Êý¾Ý·Åµ½Ò»¸öÍ³Ò»¹ÜÀíµÄÆ¬ÄÚÎÂ¶ÈQUEÀïÒÔ±ã×öÆ½¾ù´¦Àí
            }
            else
            {
