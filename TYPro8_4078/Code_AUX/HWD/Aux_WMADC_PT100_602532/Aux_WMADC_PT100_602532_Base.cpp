...
void Aux_WMADC_PT100_602532_Base::Hardware_Init(unsigned char BoardID)
{
    ...
    //AuxDataBank[0]对应第一个ADC(1-8通道--AuxDataBank[0][0]-AuxDataBank[0][8])
    //AuxDataBank[1]对应第二个ADC(17-24),AuxDataBank[2]对应第三个ADC(33-40),AuxDataBank[3]对应第四个ADC(49-56)
...
{
    SPI_SET();
    unsigned long code = Adc32bit_LTC2449::Read(Aux_DataManager::Aux_ADC_CS_Info[u_CSid].sspChn); //将数据放到Aux_DataBank 中
    if(code != 0)
		{
