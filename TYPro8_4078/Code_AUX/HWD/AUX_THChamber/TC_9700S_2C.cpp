...
void TC_9700S_2C::Init(unsigned char ChanNo)
{
    // 初始化函数指针。
    unsigned char uc_i; // 防止死机情况，先全部初始化一次函数指针。
    for(uc_i = 0; uc_i < MAX_COM_CMD_TOTAL; uc_i++)
    {
        st_SendCMD[ChanNo][uc_i].Function = (&TC_9700S_2C::SendRunCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
    }

    st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TC_9700S_2C::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TC_9700S_2C::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TC_9700S_2C::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TC_9700S_2C::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TC_9700S_2C::Parse);  //  5-S)读取温度从机回复主机命令  (以字节操作)
...
*   Return: void                                                  *
***************************************************************/
void TC_9700S_2C::Parse(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
		int temp;
    short PresentValue = 0;
    unsigned short int uw16_i = 0;

    if( UARTFunc::uart_bytenum(uc_ChNo) < TC_9700S_2C_MIN_RESPONSE_CMD_LEN) //wait until MIN_LEN bytes have been read into the buffer
        return;
...
// 						MTCI_CTRL::SetPresentValue(uc_ChNo,PresentValue,SETPOINTVALUE_HUMI, FIRST_CHAMBER_CHANNEL);
        ...
        uw16_i++;
    } while(uw16_i < TC_9700S_2C_MIN_RESPONSE_CMD_LEN);
}
...
void TC_9700S_2C::SendWriteCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
...
// 		{
			...
			CmdBuf[ADDR_LOW] = 0x03;//modify 20210901
...
void TC_9700S_2C::SendReadCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
    unsigned char CmdBuf[SEND_CMD_BUF_SIZE];
		unsigned short crc16;
...
void TC_9700S_2C::SendRunCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
    unsigned char CmdBuf[SEND_CMD_BUF_SIZE];
		unsigned short crc16;
...
*   Out: void                                                  *
***************************************************************/
void TC_9700S_2C::SendStopCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
    unsigned char CmdBuf[SEND_CMD_BUF_SIZE];
		unsigned short crc16;
