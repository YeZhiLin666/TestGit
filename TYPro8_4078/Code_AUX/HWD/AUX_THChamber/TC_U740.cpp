...
void TC_U740::Init(unsigned char ChanNo)
{
// 初始化函数指针。
    unsigned char uc_i; // 防止死机情况，先全部初始化一次函数指针。
    for(uc_i = 0; uc_i < MAX_COM_CMD_TOTAL; uc_i++)
    {
        st_SendCMD[ChanNo][uc_i].Function = (&TC_U740::SendReadCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
    }

    st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TC_U740::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TC_U740::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TC_U740::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TC_U740::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TC_U740::ParseResult);  //  5-S)读取温度从机回复主机命令  (以字节操作)

    MTCI_CTRL::uw16_PUBLIC_REPEAT_NUM = 1; // PUBLIC_REPEAT_NUM;
    MTCI_CTRL::uw32_CMD_INTERVALTIME = 200*10; //CMD_INTERVALTIME;
}

void TC_U740::ParseResult(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
// 分析接收的结果数据
    ...
    if( uw16_Total < TC_U740_MIN_RESPONSE_CMD_LEN)
        return;
...
void TC_U740::SendWriteCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
  unsigned char SetTempCmd[] = {0x01,0x06,0x00,0x64,0x00,0x00,0xff,0xff};//设定温度通道值
// 	unsigned char SetHumiCmd[] = {0x01,0x06,0x00,0x65,0x00,0x00,0xff,0xff};//设定湿度通道值
...
void TC_U740::SendReadCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
  unsigned char ReadTempCmd[] = {0x01,0x03,0x00,0x0c,0x00,0x01,0xff,0xff};//读取1个通道温度值
// 	unsigned char ReadHumiCmd[] = {0x01,0x03,0x00,0x11,0x00,0x01,0xff,0xff};//读取1个通道湿度值
	unsigned short crc16;
...
void TC_U740::SendRunCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
...
void TC_U740::SendStopCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
