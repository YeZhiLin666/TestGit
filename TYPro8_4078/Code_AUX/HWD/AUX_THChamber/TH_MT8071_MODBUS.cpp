...
// 初始化函数指针。
    ...
    {
        st_SendCMD[ChanNo][uc_i].Function = (&TH_MT8071_MODBUS::SendReadCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
    }

    st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TH_MT8071_MODBUS::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TH_MT8071_MODBUS::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TH_MT8071_MODBUS::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TH_MT8071_MODBUS::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TH_MT8071_MODBUS::ParseResult);  //  5-S)读取温度从机回复主机命令  (以字节操作)
...
void TH_MT8071_MODBUS::SendWriteCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
    ...
    unsigned char SetHumiCmd[] = {0x01,0x06,0x1F,0xA5,0x00,0x00,0xff,0xff};//设定湿度通道值
...
void TH_MT8071_MODBUS::SendReadCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
    ...
    unsigned char ReadHumiCmd[] = {0x01,0x03,0x1F,0x38,0x00,0x01,0xff,0xff};//读取1个通道湿度值
    unsigned short crc16;
