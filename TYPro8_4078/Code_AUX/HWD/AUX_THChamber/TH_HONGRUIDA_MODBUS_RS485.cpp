...
// 初始化函数指针。
    ...
    {
        st_SendCMD[ChanNo][uc_i].Function = (&TH_HONGRUIDA_MODBUS_RS485::SendReadCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
    }

    st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TH_HONGRUIDA_MODBUS_RS485::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TH_HONGRUIDA_MODBUS_RS485::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TH_HONGRUIDA_MODBUS_RS485::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TH_HONGRUIDA_MODBUS_RS485::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TH_HONGRUIDA_MODBUS_RS485::ParseResult);  //  5-S)读取温度从机回复主机命令  (以字节操作)
}
...
void TH_HONGRUIDA_MODBUS_RS485::SendWriteCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
    ...
    {
        unsigned char SetTempCmd[] = {0x01,0x10,0x00,0x87,0x00,0x01,0x02,0xff,0xff,0xff,0xff};//写1个通道温度设定值
		...
		{
        unsigned char SetHumiCmd[] = {0x01,0x10,0x00,0x89,0x00,0x01,0x02,0xff,0xff,0xff,0xff};//写1个通道温度设定值
...
void TH_HONGRUIDA_MODBUS_RS485::SendReadCmd(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
    ...
    if(uc_Temp4Humi_Flag == SETPOINTVALUE_TEMPERATURE)
    {
        ...
        unsigned char ReadTempCmd[] = {0x01,0x03,0x00,0x79,0x00,0x01,0xff,0xff};//读取1个通道温度值
    ...
    else if(uc_Temp4Humi_Flag == SETPOINTVALUE_HUMI)
    {
        ...
        unsigned char ReadHumiCmd[] = {0x01,0x03,0x00,0x81,0x00,0x01,0xff,0xff};//读取1个通道流量值
...
{
    unsigned char MbBuf[] = {0x01,0x10,0x00,0x0E,0x00,0x01,0x02,0x01,0x01,0x67,0x2E};//启动定值1#
...
{
    unsigned char MbBuf[] = {0x01,0x10,0x00,0x0E,0x00,0x01,0x02,0x00,0x00,0xA7,0x7E};//停止定值1#
    unsigned short uw16_len = sizeof(MbBuf);
