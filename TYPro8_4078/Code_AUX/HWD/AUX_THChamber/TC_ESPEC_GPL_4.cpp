...
void TC_GPL_4::Init(unsigned char ChanNo)
{
	// 初始化函数指针。
	unsigned char uc_i; // 防止死机情况，先全部初始化一次函数指针。
	for(uc_i = 0; uc_i < MAX_COM_CMD_TOTAL; uc_i++)
	{
		st_SendCMD[ChanNo][uc_i].Function = (&TC_GPL_4::SendRunCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
	}

	st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TC_GPL_4::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
	st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TC_GPL_4::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
	st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TC_GPL_4::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
	st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TC_GPL_4::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)  
	st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TC_GPL_4::Parse);  //  5-S)读取温度从机回复主机命令  (以字节操作)
...
void TC_GPL_4::Parse(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
	unsigned char *pCmd;
	...
	if( UARTFunc::uart_bytenum(uc_ChNo) < TC_GPL_4_MIN_RESPONSE_CMD_LEN) //wait until MIN_LEN bytes have been read into the buffer
		return;

	memset(MTCI_CTRL::gR_CmdBuf,0,sizeof(MTCI_CTRL::gR_CmdBuf));
  UARTFunc::uart_getcmd(uc_ChNo,MTCI_CTRL::gR_CmdBuf);    

	MTCI_CTRL::gR_CmdBuf[TC_GPL_4_MAX_RESPONSE_CMD_LEN+1]=0;
...
//			MTCI_CTRL::SetPresentValue(uc_ChNo, PresentValue,SETPOINTVALUE_HUMI); 
//		}
...
void TC_GPL_4::SendWriteCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
	unsigned char  *p;
...
//			short SetPointValue=MTCI_CTRL::gW_HumiValue[uc_ChNo];
//			unsigned short Count = 0; 
...
void TC_GPL_4::SendRunCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
	unsigned char *pCmd;
...
void TC_GPL_4::SendReadCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
	unsigned char *pCmd;
...
*   Out: void                                                  *
***************************************************************/
void TC_GPL_4::SendStopCmd(unsigned char uc_ChNo,unsigned char uc_Temp4Humi_Flag)
{
	unsigned char *pCmd;
