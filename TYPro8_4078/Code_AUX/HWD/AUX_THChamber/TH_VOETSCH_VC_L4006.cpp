...
const unsigned char uc_COM_WriteCmd_VOETSCH_L4006[TH_VOETSCH_L4006_MAX_COM_WRITE_CMD] =
    "$01E 0022.0 0050.0 01101010101010101010101010101010";     // write Master to  Slave cmd  (设置温度22度)
...
const unsigned char uc_COM_ReadCmd_VOETSCH_L4006[TH_VOETSCH_L4006_MAX_COM_READ_CMD] = "$01I";    // read Master to  Slave cmd (读取)
...
const unsigned char uc_COM_RunCmd_VOETSCH_L4006[TH_VOETSCH_L4006_MAX_COM_RUN_CMD] = "$01I";    // read Master to  Slave cmd (读取)
...
const unsigned char uc_COM_StopCmd_VOETSCH_L4006[TH_VOETSCH_L4006_MAX_COM_STOP_CMD] =
    "$01E 0022.0 0050.0 00101010101010101010101010101010";    // write Master to  Slave cmd  (设置温度22度)
...
void TH_VOETSCH_L4006::Init(unsigned char ChanNo)
{
    ...
    {
        st_SendCMD[ChanNo][uc_i].Function = (&TH_VOETSCH_L4006::SendReadCmd);   //  2-M)读取温度主机到从机命令  (以字节操作)
    }

    st_SendCMD[ChanNo][SEND_COM_WRITE_CMD].Function = (&TH_VOETSCH_L4006::SendWriteCmd);  //  1-M)设置温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_READ_CMD].Function = (&TH_VOETSCH_L4006::SendReadCmd);    //  2-M)读取温度主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_RUN_CMD].Function = (&TH_VOETSCH_L4006::SendRunCmd);      //  3-M)要求运行命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][SEND_COM_STOP_CMD].Function = (&TH_VOETSCH_L4006::SendStopCmd);    //  4-M)要求停止命令主机到从机命令  (以字节操作)
    st_SendCMD[ChanNo][RECEIVE_COM_READ_CMD].Function = (&TH_VOETSCH_L4006::ParseResult);  //  5-S)读取温度从机回复主机命令  (以字节操作)
}
...
void TH_VOETSCH_L4006::ParseResult(unsigned char ChanNo,unsigned char uc_Temp4Humi_Flag)
{
    ...
    {   //  控制在正190度负55度。  // Temperature range -55°C to +190°C //Resolution 0.1°C or °F, 0.1% RH, or 0.01 for other linear applications
        if(MTCI_CTRL::gR_CmdBuf[uw16_i] == 0X0D)  // 寻找End <CR>，过滤多余的位数。
        {
            ...
            if(MTCI_CTRL::gR_CmdBuf[uw16_i + 61] != 0X0D)  // 寻找End <CR>，过滤多余的位数。
    ...
    if(MTCI_CTRL::gR_CmdBuf[uw16_k + 61] != 0X0D)  // 寻找End <CR>，过滤多余的位数。
...
// 	MTCI_datatypes::ctrl_Value2String_BeforeZero(&register_data,MbBuf + TH_VOETSCH_L4006_WRITE_CMD_POINT_VAlUE + 15);  // Set value 1
    ...
    for(MbChkSum = 0; MbChkSum < 30; MbChkSum++)
    {
        ...
        {
            MbBuf[TH_VOETSCH_L4006_MAX_COM_WRITE_CMD-2-MbChkSum]= '1';  // 置1
        }
        else
        {
            MbBuf[TH_VOETSCH_L4006_MAX_COM_WRITE_CMD-2-MbChkSum]= '0';  // 置0
        }
    }
